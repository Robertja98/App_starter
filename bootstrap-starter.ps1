...
$replacements = @(
    ...
    @{ Path = Join-Path $scriptPath 'config\app.php'; Old = "'database' => getenv('DB_NAME') ?: 'app_local_dev'"; New = "'database' => getenv('DB_NAME') ?: '" + $DatabaseName + "'" },
    @{ Path = Join-Path $scriptPath 'config\app.php'; Old = "'name'      => getenv('APP_NAME') ?: 'My App Starter'"; New = "'name'      => getenv('APP_NAME') ?: '" + $AppName + "'" },
    @{ Path = Join-Path $scriptPath 'config\app.php'; Old = "'short_name' => getenv('APP_SHORT_NAME') ?: 'My App'"; New = "'short_name' => getenv('APP_SHORT_NAME') ?: '" + $workspaceTitle + "'" },
    @{ Path = Join-Path $scriptPath 'config\app.php'; Old = "'description' => getenv('APP_DESCRIPTION') ?: 'Offline-capable PHP and MySQL application starter'"; New = "'description' => getenv('APP_DESCRIPTION') ?: '" + $AppName + " application'" },
    @{ Path = Join-Path $scriptPath 'config\app.php'; Old = "'name' => getenv('SESSION_NAME') ?: 'app_session'"; New = "'name' => getenv('SESSION_NAME') ?: '" + $sessionName + "'" },
    @{ Path = Join-Path $scriptPath 'config\app.php'; Old = "'api_key_prefix' => getenv('API_KEY_PREFIX') ?: 'app_'"; New = "'api_key_prefix' => getenv('API_KEY_PREFIX') ?: '" + $apiKeyPrefix + "'" },
    @{ Path = Join-Path $scriptPath 'setup-local-db.ps1'; Old = '[string]$DbName = "app_local_dev"'; New = '[string]$DbName = "' + $DatabaseName + '"' },
    ...
    @{ Path = Join-Path $scriptPath 'public\app.html'; Old = 'Offline-capable PHP and MySQL application starter'; New = $AppName + ' web application' },
    @{ Path = Join-Path $scriptPath 'public\app.html'; Old = 'My App'; New = $workspaceTitle },
    @{ Path = Join-Path $scriptPath 'public\manifest.json'; Old = 'My App Starter'; New = $AppName },
    @{ Path = Join-Path $scriptPath 'public\manifest.json'; Old = 'My App'; New = $workspaceTitle },
    @{ Path = Join-Path $scriptPath 'public\manifest.json'; Old = 'Offline-capable PHP and MySQL application starter'; New = $AppName + ' application' },
    @{ Path = Join-Path $scriptPath 'dev-server.ps1'; Old = 'PHP MySQL PWA Starter - Dev Server'; New = $AppName + ' - Dev Server' },
    @{ Path = Join-Path $scriptPath 'dev-server.ps1'; Old = '  2. Database created: CREATE DATABASE app_local_dev;'; New = '  2. Database created: CREATE DATABASE ' + $DatabaseName + ';' },
    @{ Path = Join-Path $scriptPath 'dev-server.ps1'; Old = '  3. Schema imported: mysql -u root app_local_dev < database/migrations/001_initial_schema.sql'; New = '  3. Schema imported: mysql -u root ' + $DatabaseName + ' < database/migrations/001_initial_schema.sql' },
    @{ Path = Join-Path $scriptPath 'dev-server.sh'; Old = 'PHP MySQL PWA Starter - Dev Server'; New = $AppName + ' - Dev Server' },
    @{ Path = Join-Path $scriptPath 'dev-server.sh'; Old = "  2. Database created: mysql -u root -e 'CREATE DATABASE app_local_dev;'"; New = "  2. Database created: mysql -u root -e 'CREATE DATABASE " + $DatabaseName + ";'" },
    @{ Path = Join-Path $scriptPath 'dev-server.sh'; Old = '  3. Schema imported: mysql -u root app_local_dev < database/migrations/001_initial_schema.sql'; New = '  3. Schema imported: mysql -u root ' + $DatabaseName + ' < database/migrations/001_initial_schema.sql' },
    @{ Path = Join-Path $scriptPath 'LOCAL_TESTING_GUIDE.md'; Old = 'app_local_dev'; New = $DatabaseName },
    @{ Path = Join-Path $scriptPath 'LOCAL_TESTING_GUIDE.md'; Old = 'app_session'; New = $sessionName },
